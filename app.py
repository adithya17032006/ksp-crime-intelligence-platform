...
from data_processor import extract_features_from_unstructured_text, read_pdf_bytes_to_string
...
async def predict_risk_from_raw_document(file: UploadFile = File(...)):
    ...
    extracted_data = extract_features_from_unstructured_text(document_text)
...
def predict_crime_risk_via_json(payload: StructuredPredictionInput):
    ...
    try:
        dt = datetime.strptime(payload.timestamp, "%Y-%m-%d %H:%M:%SS" if len(payload.timestamp) > 16 else "%Y-%m-%d %H:%M:%S")
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid date layout. Sync to: 'YYYY-MM-DD HH:MM:SS'.")
