...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Outfit:wght@400;500;600;700;800&family=IBM+Plex+Sans:wght@400;500;600;700&family=IBM+Plex+Mono:wght@400;500;600&display=swap');
...
.auth-card::before{
    ...
    background: linear-gradient(90deg, var(--saffron), var(--gold) 45%, var(--text-0) 55%, var(--green) 100%);
    opacity:.55;
}
@keyframes fadeUp{ from{opacity:0; transform:translateY(10px);} to{opacity:1; transform:translateY(0);} }

.ksp-brand { text-align:center; padding: 6px 0 26px 0; }
.ksp-crest-wrap{ display:flex; justify-content:center; margin-bottom:14px; }
...
.metric-card-hover { transition: transform .18s ease, box-shadow .18s ease, border-color .18s ease; }
.metric-card-hover:hover { transform: translateY(-4px); box-shadow: 0 14px 32px rgba(0,0,0,.55); border-color: var(--gold); }

/* ── Chat bubbles ────────────────────────────────────── */
.chat-officer {
    background: rgba(59,130,246,0.14); border: 1px solid var(--blue);
    border-radius: 12px 12px 2px 12px; padding: 10px 15px;
    margin: 6px 0; color: var(--text-0); font-size: 14px; text-align: right;
}
.chat-ai {
    background: rgba(16,185,129,0.10); border: 1px solid var(--green);
...
.rank-1 { background: rgba(148,163,184,0.18); color: var(--text-2); border:1px solid var(--text-2); }

/* ── Case / record numbers everywhere look like case numbers ── */
.case-id, code { font-family: var(--font-mono) !important; }
...
.stButton > button[kind="primary"]{
    background: linear-gradient(135deg, var(--gold), #ca8a04) !important;
...
section[data-testid="stSidebar"] .stRadio [role="radiogroup"] > label:hover{
    background: rgba(234,179,8,0.08); color: var(--gold-soft);
}
...
def render_crest(size: int = 84) -> str:
    ticks = "".join(
        f'<line x1="42" y1="6" x2="42" y2="12" transform="rotate({i*15} 42 42)" '
        f'stroke="#eab308" stroke-width="1.6" stroke-linecap="round" opacity="0.85"/>'
        for i in range(24)
    )
    return f"""
    <div class="ksp-crest-wrap">
...
def render_register_page():
    ...
    with col_c:
        ...
        <div class="ksp-brand">
            ...
            <p style="color:#94a3b8; font-size:14px; margin:6px 0 0 0;">Karnataka State Police &middot; New Officer Enrollment</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:10px;">
            ...
            <div>
                <div style="background:rgba(234,179,8,0.15); border:1px solid #eab308; padding:6px 14px;
                            ...
                            text-transform:uppercase; letter-spacing:.5px; font-family:'IBM Plex Mono';">{title_suffix}</div>
...
def draw_kpi_card(title, value, subtitle, border_color="#3B82F6"):
    ...
    <div class="metric-card-hover" style="background:linear-gradient(135deg,#0f172a 0%,#1e293b 100%);
         border-top:4px solid {border_color}; border-radius:10px; padding:20px;
         box-shadow:0 4px 15px rgba(0,0,0,.3); border:1px solid #334155;
         border-top-color:{border_color}; height:100%;">
        <h4 style="margin:0; color:#94a3b8; font-size:11px; font-weight:600; text-transform:uppercase; letter-spacing:.04em;">{title}</h4>
        <h2 style="margin:10px 0 4px 0; font-size:30px; font-weight:800; color:#f8fafc; font-family:'Outfit';">{value}</h2>
